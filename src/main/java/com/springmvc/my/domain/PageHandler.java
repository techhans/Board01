package com.springmvc.my.domain;
...
//    private String  keyword;
    ...
    private void doPaging(int totalCnt, SearchCondition sc) {
    	int ans1 = totalCnt / sc.getPageSize();
    	int ans2 = totalCnt % sc.getPageSize()==0? 0:1;
        this.totalPage = totalCnt / sc.getPageSize() + (totalCnt % sc.getPageSize()==0? 0:1);
    	int ans3 = sc.getPage();
    	int ans4 = Math.min(sc.getPage(), totalPage);
        this.sc.setPage(Math.min(sc.getPage(), totalPage));  // page가 totalPage보다 크지 않게
      	int ans5 = (this.sc.getPage() -1);
      	int ant6 = NAV_SIZE * NAV_SIZE + 1;
